...
public class Java2DImage implements Image {
	...
	public Java2DImage(java.awt.image.BufferedImage image) {
		this.image = image;
	}

	@Override
	public void draw(GraphicsContext context, int x, int y) {
		((Java2DGraphicsContext)context).getGraphicsContext().drawImage(image, x, y, null);
	}
	
	@Override
	public void draw(GraphicsContext context, int x, int y, int w, int h) {
		((Java2DGraphicsContext)context).getGraphicsContext().drawImage(image, x, y, w, h, null);
	}
