...
public class DSLOperatorMul extends DSLOperator {
	public DSLOperatorMul(DSLToken token, DSLExpression exp1, DSLExpression exp2, int numberIndex) {
	...
	public ComplexNumber evaluate(DSLInterpreterContext context, Map<String, Variable> scope) {
		return context.getNumber(numberIndex).set(opMul(exp1.evaluateReal(context, scope), exp2.evaluateReal(context, scope)));
	}
