...
public class DSLFunctionImZ extends DSLFunction {
	public DSLFunctionImZ(DSLToken token, DSLExpressionContext context, DSLExpression[] arguments) {
	...
	public ComplexNumber evaluate(DSLInterpreterContext context, Map<String, Variable> scope) {
		return context.getNumber(numberIndex).set(funcIm(arguments[0].evaluate(context, scope)));
	}
	...
	public void compile(DSLCompilerContext context, Map<String, VariableDeclaration> scope) {
		CompilerUtils.compileComplexFunctionOneComplexArgument(token, context, scope, "funcIm", arguments);
	}
}
