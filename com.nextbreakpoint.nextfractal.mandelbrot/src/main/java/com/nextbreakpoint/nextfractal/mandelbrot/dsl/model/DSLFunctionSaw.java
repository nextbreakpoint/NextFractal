...
public class DSLFunctionSaw extends DSLFunction {
	public DSLFunctionSaw(DSLToken token, DSLExpressionContext context, DSLExpression[] arguments) {
	...
	public ComplexNumber evaluate(DSLInterpreterContext context, Map<String, Variable> scope) {
		return context.getNumber(numberIndex).set(funcSaw(arguments[0].evaluateReal(context, scope)));
	}
	...
	public void compile(DSLCompilerContext context, Map<String, VariableDeclaration> scope) {
		CompilerUtils.compileRealFunctionOneArgument(token, context, scope, "funcSaw", arguments);
	}
}
