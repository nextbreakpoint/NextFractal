...
public class DSLFunctionFloor extends DSLFunction {
	public DSLFunctionFloor(DSLToken token, DSLExpressionContext context, DSLExpression[] arguments) {
	...
	public ComplexNumber evaluate(DSLInterpreterContext context, Map<String, Variable> scope) {
		return context.getNumber(numberIndex).set(funcFloor(arguments[0].evaluateReal(context, scope)));
	}
	...
	public void compile(DSLCompilerContext context, Map<String, VariableDeclaration> scope) {
		CompilerUtils.compileRealFunctionOneArgument(token, context, scope, "funcFloor", arguments);
	}
}
