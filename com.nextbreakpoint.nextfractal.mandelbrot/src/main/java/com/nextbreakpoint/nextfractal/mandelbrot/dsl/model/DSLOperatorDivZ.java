...
public class DSLOperatorDivZ extends DSLOperator {
	public DSLOperatorDivZ(DSLToken token, DSLExpression exp1, DSLExpression exp2, int numberIndex) {
	...
	public ComplexNumber evaluate(DSLInterpreterContext context, Map<String, Variable> scope) {
		return opDiv(context.getNumber(numberIndex), exp1.evaluate(context, scope), exp2.evaluate(context, scope));
	}
	...
	public void compile(DSLCompilerContext context, Map<String, VariableDeclaration> scope) {
		CompilerUtils.compileComplexMathOperator(context, scope, "opDiv", exp1, exp2, numberIndex);
	}
}
