...
public class DSLFunctionCos extends DSLFunction {
	public DSLFunctionCos(DSLToken token, DSLExpressionContext context, DSLExpression[] arguments) {
	...
	public ComplexNumber evaluate(DSLInterpreterContext context, Map<String, Variable> scope) {
		return context.getNumber(numberIndex).set(funcCos(arguments[0].evaluateReal(context, scope)));
	}
	...
	public void compile(DSLCompilerContext context, Map<String, VariableDeclaration> scope) {
		CompilerUtils.compileComplexFunctionOneArgument(token, context, scope, "funcCos", arguments, numberIndex);
	}
}
