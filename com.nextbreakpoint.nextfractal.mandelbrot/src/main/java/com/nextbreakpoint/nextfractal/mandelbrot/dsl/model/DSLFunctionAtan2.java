...
public class DSLFunctionAtan2 extends DSLFunction {
	public DSLFunctionAtan2(DSLToken token, DSLExpressionContext context, DSLExpression[] arguments) {
	...
	public ComplexNumber evaluate(DSLInterpreterContext context, Map<String, Variable> scope) {
		return context.getNumber(numberIndex).set(funcAtan2(arguments[0].evaluateReal(context, scope), arguments[1].evaluateReal(context, scope)));
	}
	...
	public void compile(DSLCompilerContext context, Map<String, VariableDeclaration> scope) {
		CompilerUtils.compileRealFunctionTwoArguments(token, context, scope, "funcAtan2", arguments);
	}
}
