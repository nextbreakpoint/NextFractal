...
public class DSLFunctionReZ extends DSLFunction {
	public DSLFunctionReZ(Token location, ExpressionContext context, DSLExpression[] arguments) {
	...
	public ComplexNumber evaluate(DSLInterpreterContext context, Map<String, Variable> scope) {
		return context.getNumber(numberIndex).set(funcRe(arguments[0].evaluate(context, scope)));
	}
	...
	public void compile(DSLCompilerContext context, Map<String, VariableDeclaration> scope) {
		DSLUtils.compileComplexFunctionOneComplexArgument(location, context, scope, "funcRe", arguments);
	}
}
