...
public class CompiledFuncRamp extends CompiledExpression {
	private final CompiledExpression[] arguments;

	public CompiledFuncRamp(ExpressionCompilerContext context, CompiledExpression[] arguments, Token location) {
	...
	public Number evaluate(ExpressionContext context, Map<String, Variable> scope) {
		return context.getNumber(index).set(funcRamp(arguments[0].evaluateReal(context, scope)));
	}
