...
public class DSLFunctionMod2 extends DSLFunction {
	public DSLFunctionMod2(DSLToken token, DSLExpressionContext context, DSLExpression[] arguments) {
	...
	public ComplexNumber evaluate(DSLInterpreterContext context, Map<String, Variable> scope) {
		return context.getNumber(numberIndex).set(funcMod2(arguments[0].evaluateReal(context, scope)));
	}
	...
	public void compile(DSLCompilerContext context, Map<String, VariableDeclaration> scope) {
		CompilerUtils.compileComplexFunctionOneComplexArgument(token, context, scope, "funcMod2", arguments);
	}
}
